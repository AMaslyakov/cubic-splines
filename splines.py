...
def tridiagonal_coefficients(input_data):
    ...
    for i in range(1, n):
        ...
        # поэтому индексы массива Y понижены на 1 для того чтобы формула коэффицентов значений, была корректной
        answer = 3 * (((y[i + 1] - y[i]) / h_i[i]) - ((y[i] - y[i - 1]) / h_i[i - 1]))
        #logger.debug(f"{main_diagonal=}; {down_diagonal=}; {upper_diagonal=}; {answer=}")
        if i == 1:
...
def calculate_c_i(U:list[float], V:list[float]):
    """
    :param U: Коэффициенты прогонки U
    :param V: Коэффициенты прогонки V
